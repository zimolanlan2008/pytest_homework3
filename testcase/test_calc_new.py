...
class TestCalc:
    ...
    # 增加标签在ini文件中提前写好,通过标签方式给测试用例分类
    @pytest.mark.add
    def test_add(self, get_calc, get_data_add):
        a = get_data_add[0]
        b = get_data_add[1]
        expect = get_data_add[2]
    ...
    def test_div(self, get_calc, get_data_div):
        a = get_data_div[0]
        b = get_data_div[1]
        expect = get_data_div[2]
        # 判断b不能等于0
        if b == 0:
            print("除数不能等于0")
            assert False
    ...
    def test_sub(self, a, b, expect, get_calc, caculate):
